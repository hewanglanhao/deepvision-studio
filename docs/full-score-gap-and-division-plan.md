...
3. **A 模式训练区域是 Mock**
   - A 模式虽然有训练面板，但 HTML 明确写着“Mock Training”，TS 中调用的是 `trainingSvc.start()`，不是 `startBackend()`。
   - 所以 A 不能作为真实训练得分来源，只能作为前向传播/卷积核影响/单样本推理教学模式。
...
- 没有真正的“任意节点连线/删除边/分支网络”编辑体验。
- 不支持 ResNet 残差连接这类分支结构。

建议：

- 若冲满分，至少在 A 模式做“连接编辑增强”：选中源层、选中目标层、生成连接线；支持删除连接。
- 若时间紧，可以不做复杂 DAG，只做“残差连接开关/Skip Connection 可视化”，用于进阶展示。
...
- B 可加“ResNet Block 模板”或“Skip Connection 可视化”，但如果网络编辑器不支持分支，先做展示型残差块即可。
- 全局加 WebSocket 简单聊天室：同一个 `jobId` 房间，允许用户发消息讨论训练过程。
- 部署阶段加 Nginx HTTPS/HTTP2，写入部署文档。

负责人建议：C 做自己模式内的 Grad-CAM；A 可做前向传播结构层面的 Skip Connection 展示；B 可做训练模板层面的高级结构选项；全局工程做协作和部署。各模式只共享组件和类型，不共享训练结果。
...
1. A 页面包含训练区域，但它是 Mock。
   - 容易和 B 模式真实训练混淆。
   - 建议 A 页面保留“前向传播”主线，训练区要么隐藏，要么明确是“辅助演示，不计入真实训练”。

2. A 的 forward 权重不是训练后模型权重。
   - 这是符合分工边界的：A 只解释“前向传播怎么算”，不负责展示 B 训练出的权重。

3. 网络连接能力偏线性。
   - 满分项中的“连接网络层”可以解释为线性连接已做，但若老师严格看“任意连接”，还不够。
...
- 把 A 的训练区隐藏或移动到“旧版 Mock 演示”折叠区，避免影响答辩判断。
...
- 对每一层展示：
  ...
  - 当前层参数摘要。
- 增加“逐层播放”按钮：Input -> Conv -> Activation -> Pool -> Flatten -> Dense -> Output。
...
- 支持删除连接。
- 在 UI 上显示非线性连接或残差连接。
- 如果工程量大，先做一个“Skip Connection 展示开关”，为 B/C 进阶结构服务。
...
- A-P0 稳定 A 模式前向传播演示。
- A-P0 修复 A 页面和首页入口相关乱码。
- A-P1 隐藏/弱化 A 的 Mock 训练区。
- A-P1 强化 A 内部单样本逐层前向传播展示。
- A-P1 增强网络连接/Skip Connection 可视化。
- A-P2 优化 A 模式 AI 助手预设问题。
...
1. C 实现 Grad-CAM。
2. D 实现/打磨决策边界与优化器对比。
3. A 或 B 实现 Skip Connection/残差块可视化。
4. B WebSocket 协作讨论。
...
5. **全局能力**
   - 登录、记录、LLM 助手、WebSocket 协作、Docker/云部署、文档和测试。

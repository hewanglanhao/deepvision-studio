...
const MODE_D_LLM_SYSTEM_PROMPT = `你是 DeepVision Studio 模式 D 的 AI 教学助手。当前用户在观察一个 MLP 在二维数据上进行反向传播训练的完整过程。
...
export class ModeDExplainerShellComponent implements OnInit, OnDestroy {
  ...
  modeDLlmContextProvider = (): LlmChatContext => {
    return this.buildModeDLlmContext();
  };

  private buildModeDLlmContext(): LlmChatContext {
